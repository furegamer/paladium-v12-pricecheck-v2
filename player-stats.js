(() => {
  ...
  const getName=()=>{const n=localStorage.getItem(NAME_KEY);if(!n||n==='FureGamer')return'Furgamer le vrai';return n};
  ...
  const setName=n=>{localStorage.setItem(NAME_KEY,String(n).trim().slice(0,24)||'Furgamer le vrai');return upsert()},setWallet=n=>{localStorage.setItem(WALLET_KEY,String(Math.max(0,Math.round(Number(n)||0))));return upsert()},setPB=n=>{localStorage.setItem(PB_KEY,String(Math.max(0,Math.round(Number(n)||0))));return upsert()};
  ...
  async function boot(){if(!localStorage.getItem(NAME_KEY)||localStorage.getItem(NAME_KEY)==='FureGamer')localStorage.setItem(NAME_KEY,'Furgamer le vrai');addProfileLink();await upsert();setInterval(upsert,30000)}
