(() => {
  ...
  const dedupeProfileLinks=()=>document.querySelectorAll('.links').forEach(l=>{const links=[...l.querySelectorAll('a[href="profile.html"]')];links.slice(1).forEach(a=>a.remove())});
  async function boot(){dedupeProfileLinks();await upsert();setInterval(upsert,30000);setInterval(dedupeProfileLinks,1000)}
