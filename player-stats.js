(() => {
  ...
  async function upsert(){
    ...
    const body={player_id:getId(),display_name:getName(),wallet:getWallet(),pb:getPB(),miner_level:Number(levels.Mineur||1),farmer_level:Number(levels.Fermier||1),hunter_level:Number(levels.Chasseur||1),alchemist_level:Number(levels.Alchimiste||1),pog_level:Number(localStorage.getItem(POG_KEY)||1),last_seen:new Date().toISOString()};
  ...
  async function boot(){addProfileLink();await upsert();setInterval(upsert,30000);window.PriceCheckPlayers={upsert,api,getId,getName,getWallet,getPB,setName:n=>{localStorage.setItem(NAME_KEY,String(n).trim().slice(0,24)||getName());return upsert()},setWallet:n=>{localStorage.setItem(WALLET_KEY,String(Math.max(0,Math.round(Number(n)||0))));return upsert()},setPB:n=>{localStorage.setItem(PB_KEY,String(Math.max(0,Math.round(Number(n)||0))));return upsert()}}}
