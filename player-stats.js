(() => {
  ...
  const getName=()=>localStorage.getItem(NAME_KEY)||'Furgamer le vrai';
  ...
  const setName=n=>{localStorage.setItem(NAME_KEY,String(n).trim().slice(0,24)||'Furgamer le vrai');return upsert()};
  const setWallet=n=>{localStorage.setItem(WALLET_KEY,String(Math.max(0,Math.round(Number(n)||0))));return upsert()};
  const setPB=n=>{localStorage.setItem(PB_KEY,String(Math.max(0,Math.round(Number(n)||0))));return upsert()};
