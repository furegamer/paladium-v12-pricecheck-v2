(() => {
  ...
  style.textContent = `
    .pc-hud{position:fixed;top:14px;right:14px;z-index:9999;display:flex;align-items:center;gap:8px;font-family:Inter,system-ui,-apple-system,"Segoe UI",sans-serif;pointer-events:none}
    .pc-hud *{box-sizing:border-box}
    .pc-wallet,.pc-hud-btn{pointer-events:auto;border:1px solid rgba(148,163,184,.22);box-shadow:0 12px 32px rgba(0,0,0,.35),inset 0 1px rgba(255,255,255,.06);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px)}
    .pc-wallet{display:flex;align-items:center;gap:10px;min-width:190px;padding:9px 12px;border-radius:14px;background:linear-gradient(135deg,rgba(14,20,33,.96),rgba(27,19,49,.94));color:#fff}
    ...
    .pc-hud-btn{width:40px;height:40px;border-radius:12px;background:rgba(10,15,25,.9);color:#e2e8f0;cursor:pointer;font-size:17px}.pc-hud-btn:hover{transform:translateY(-1px);border-color:rgba(139,92,246,.6);background:rgba(24,29,45,.96)}
    ...
    @media(max-width:640px){.pc-hud{top:8px;right:8px}.pc-wallet{min-width:156px;padding:7px 9px}.pc-wallet-icon{width:30px;height:30px}.pc-wallet-value{font-size:14px}.pc-hud-btn{width:36px;height:36px}.pc-wallet-label{font-size:8px}}
  `;
  ...
  const mount = () => {
    ...
    hud.innerHTML = `
      ...
      <button class="pc-hud-btn" type="button" data-wallet-open aria-label="Modifier mon solde">⚙️</button>`;
    ...
    hud.querySelector('[data-wallet-open]').addEventListener('click', open);
