...
const saveFavorites=s=>localStorage.setItem(favKey,JSON.stringify([...s]));
const history=()=>JSON.parse(localStorage.getItem(historyKey)||'{}');
const saveHistory=h=>localStorage.setItem(historyKey,JSON.stringify(h));
const card=(x)=>{const fav=favorites().has(x[0]),conf=confidence(x);return `<article class="item-card" data-name="${x[0].replaceAll('"','&quot;')}"><button class="fav ${fav?'on':''}" title="${fav?'Retirer des favoris':'Ajouter aux favoris'}" data-fav="${encodeURIComponent(x[0])}">${fav?'★':'☆'}</button><a class="item-link" href="item.html?item=${encodeURIComponent(x[0])}"><div class="item-top"><img src="${x[4]}" alt="${x[0]}" loading="lazy" onerror="this.onerror=null;this.src='images/default.png'"><span class="cat">${x[2]}</span></div><h3>${x[0]}</h3><p class="desc">${x[11]}</p><span class="rarity ${RARITY_CLASS[x[3]]}">${x[3]}</span><div class="price">${money(x[5])}</div><div class="range">${money(x[6])} → ${money(x[7])}</div><div class="confidence ${conf[1]}">Confiance ${conf[0]}</div><div class="tags">${x[10]?`<span>⭐ ${x[10]}/20</span>`:''}${x[8]!==null?`<span>🔧 ${x[8]}</span>`:''}${x[9]?`<span>⚔️ ${x[9]}</span>`:''}</div></a></article>`};
function initCatalog(){const q=$('#q'),cat=$('#cat'),rar=$('#rar'),sort=$('#sort'),favOnly=$('#favOnly'),grid=$('#items'),count=$('#count'),favCount=$('#favCount');
function render(){let s=norm(q.value),c=cat.value,r=rar.value,so=sort.value,f=favOnly.checked,fs=favorites();let a=ITEMS.filter(x=>(!s||[x[0],...x[1],x[2],x[3]].map(norm).join(' ').includes(s))&&(c==='all'||x[2]===c)&&(r==='all'||x[3]===r)&&(!f||fs.has(x[0])));if(so==='priceUp')a.sort((x,y)=>x[5]-y[5]);if(so==='priceDown')a.sort((x,y)=>y[5]-x[5]);if(so==='rarUp')a.sort((x,y)=>RARITY_ORDER[x[3]]-RARITY_ORDER[y[3]]);if(so==='rarDown')a.sort((x,y)=>RARITY_ORDER[y[3]]-RARITY_ORDER[x[3]]);if(so==='name')a.sort((x,y)=>x[0].localeCompare(y[0],'fr'));grid.innerHTML=a.length?a.map(card).join(''):`<div class="empty">😕 Aucun item ne correspond à ces filtres.</div>`;count.textContent=a.length;favCount.textContent=fs.size;$$('[data-fav]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();const set=favorites(),name=decodeURIComponent(b.dataset.fav);set.has(name)?set.delete(name):set.add(name);saveFavorites(set);render()});}
[q,cat,rar,sort,favOnly].forEach(e=>e.addEventListener(e===q?'input':'change',render));$('#clear').onclick=()=>{q.value='';cat.value='all';rar.value='all';sort.value='none';favOnly.checked=false;render()};render();}
function initMarket(){const sel=$('#marketItem'),qty=$('#qty'),dur=$('#durability'),out=$('#marketOut'),record=$('#recordPrice'),observed=$('#observedPrice'),historyBox=$('#historyBox');ITEMS.forEach(x=>{const o=document.createElement('option');o.value=x[0];o.textContent=x[0];sel.appendChild(o)});
